...
def largeNeighborhoodSearch(
        ...
        maxWithoutAcceptOrImprove=100, prctDestruct=0.1, remainingTime=60., debug=False, logs=None
        ):
    """
        ...
        if acceptFct(currentScore, reconstructedScore):
            ...
            if currentScore <= bestScore:
                if currentScore < bestScore:
                    ...
                    if debug:
                        print(Fore.BLACK + f"New best score: {currentScore} - Temps restant: "
...
def restartBestAndRandom_LNS(
    ...
    :param destructFctBest: Fonction de destruction pour le restart avec la meilleure solution, si None, on utilise destructFctRandom
    :param reconstructFctBest: Fonction de reconstruction pour le restart avec la meilleure solution, si None, on utilise reconstructFctRandom
    :param acceptFctBest: Fonction d'acceptation pour le restart avec la meilleure solution, si None, on utilise acceptFctRandom
    :param maxWithoutAcceptOrImprove: Nombre maximum d'itérations sans accepter ou améliorer avant de redémarrer
    :param prctDestruct: Pourcentage (maximum) de pièces à détruire
    :param ratioBest: Ratio x de restart avec la meilleure solution, si x > 1, on fait x restarts avec la meilleure solution avant de faire un restart aléatoire, sinon on fait 1/x restarts aléatoires avant de faire un restart avec la meilleure solution.
    :param maxTime: Temps maximum alloué à la recherche
    :param debug: Affichage des informations de débogage
    :param logs: Dictionnaire pour les logs
    :return: La meilleure solution trouvée et son score
    ...
    if logs is not None:
        ...
        logs["DestructFct"] = destructFctRandom.__name__
        logs["ReconstructFct"] = reconstructFctRandom.__name__
        logs["AcceptFct"] = acceptFctRandom.__name__
        if destructFctBest is not None:
            logs["DestructFctBest"] = destructFctBest.__name__
        if reconstructFctBest is not None:
            logs["ReconstructFctBest"] = reconstructFctBest.__name__
        if acceptFctBest is not None:
            logs["AcceptFctBest"] = acceptFctBest.__name__
        logs["NbRestart"] = nbRestart
        if nbRestartBest > 0:
